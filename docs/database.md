...
- Los modelos de `empresas`, `roles`, `permisos`, `rol_permisos`, `usuarios` y `usuario_empresa_rol` ya están implementados en `backend/prisma/schema.prisma` (Fases 3-4, con la unicidad de `usuario_empresa_rol` corregida a `(usuario, empresa)`); `cargas`/`archivos_origen`/`auditoria` desde Fase 6; `terceros` (versión mínima, sin la UI/CRUD completa de Fase 10), `documentos`, `impuestos` e `documento_impuestos` desde Fase 7 — sin `documento_detalles`, `retenciones`/`documento_retenciones` ni las FK de `documentos` a `cuenta_contable`/`centro_costo`/`forma_pago` (esos catálogos no existen todavía; se agregan cuando su fase los necesite, no antes); `reglas_validacion` y `documento_validaciones` desde Fase 8, con 7 de las 10 reglas de `validation-engine.md` implementadas (las 3 restantes dependen de columnas que `documentos` todavía no tiene).
