...
- Los modelos de `empresas`, `roles`, `permisos`, `rol_permisos`, `usuarios` y `usuario_empresa_rol` ya están implementados en `backend/prisma/schema.prisma` (Fase 3); el resto de tablas de este documento se agregan de forma incremental en las fases correspondientes (terceros en Fase 10, documentos en Fase 7, etc.).
